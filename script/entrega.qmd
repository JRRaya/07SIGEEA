...
# 1.2.1. En R
pacman::p_load(sf, terra, stringr, dplyr, tidyr, tidyverse, trend, here, ggplot2, ggridges, viridis, patchwork, tidyterra, mapview, stars, whitebox, reticulate, rgbif, vegan, betapart, adespatial, iNEXT, SpadeR, lidR, lidRmetrics)
...
# Configurar para que no aparezca
# Moverlo a otro bloque en el que se especifiquen, y luego se inserte el objeto en la funcion de rgbif
...
Para el procesamiento y manejo de estos datos LiDAR emplearemos el paquete `lidR` [@roussel2025]. En el libro dedicado a la librería [@roussel], redactado por el mismo creador que esta, viene especificado como calcular las métricas que emplearemos en nuestro análisis.
...
mde <- lapply(
  ...
  function(nombre) {
    ...
    # Guardado de los rásteres
    terra::writeRaster(
      mde,
      here::here(paste0("data/mde/mde_", nombre, ".tif")),
      overwrite = TRUE
    )
    ...
    return(list(full = mde, rec = mde_rec))
  }
)

# 4.3. Nombramos los elementos de la lista
names(mde) <- names(algoritmos)
```
...
-   **Triangulación**: El algoritmo de triangulación trabaja generando primero una TIN empleando tan solo los primero retornos (los que corresponden a las cotas más elevadas del pulso), y a continuación interpola dentro de cada triángulo generado para calcular la elevación para cada uno de los píxel del ráster.
    ...
    De nuevo, se puede recurrir a la función ya mencionada para calcular el CHM: `chm <- rasterize_canopy(ctg, res = 0.5, algorithm = dsmtin())`. Este método, al igual que el anterior, puede presentar resultados poco coherentes cuando faltan muchos puntos de la nube (como por ejemplo en terrenos que presentan muchas oquedades o masas de agua). En este caso, la solución pasa por configurar el parámetro `max_edge`, que define el número máximo de bordes de un triángulo permitos en una *triangulación de Delaunay*. Por defecto, el valor establecido es `0`, lo que significa que ningún triángulo es eliminado. Por otro lado, con un valor de (por ejemplo) `8` (`chm <- rasterize_canopy(ctg, res = 0.5, algorithm = dsmtin(max_edge = 8))`), los triángulos que contengan bordes con una longitud mayor a 8 serán eliminados (no serán tenidos en cuenta) de la triangulación.

-   **Algoritmo sin huecos (*pit-free algortihm*)**: Este algorimo más avanzado (de los que veremos), y evita la generación de huecos (píxeles sin valor de la variable) durante su cálculo, la contrario que los anteriores métodos (los cuáles requerían un postprocesamiento para poder solventar este problema).

```{r}
#| label: mds

# 1. Normalización del terreno
# 1.1. Definimos plantilla de salida de las teselas procesadas
# opt_output_files(teselas) <- paste0(here::here("data/lidar/lidar2/norm/"), "{ORIGINALFILENAME}_norm")

# 1.2. Aplicamos de forma iterativa la normalización en cada caso
teselas_n <- lapply(
  names(algoritmos),
  function(nombre) {
    opt_output_files(teselas) <- paste0(here::here(paste0("data/lidar/lidar2/norm/", nombre, "/", nombre, "_", "/")), "{ORIGINALFILENAME}")
...
consulta <- occ_download(
  pred_within(AOI_wkt),
  pred_lte("year", 2022), # Todos los registros historicos hassta el año 2022 (inclusive)
  pred("hasCoordinate", TRUE),           # Solo con coordenadas
  pred("hasGeospatialIssue", FALSE),     # Sin problemas espaciales
  format = "SIMPLE_CSV"
)

# 4.2.2. Pausamos el script hasta que la descaraga este lista
...
En nuestro caso, para calcular este parámetro emplearemos los *índices de disimilitud* (de multiples comunidades) propuestos (o sintetizados) por @chao2016 (además de otros, para poder comparar resultados). Estos son:

-   

En concreto, calcularemos todos los índices integrados en la función `betadiver()` de la librería `vegan` [@oksanen2025]. Esta función emplea como *input* la matriz transpuesta del data frame `abundanacia` empleado (en el apartado anterior) para calcular los índices de diversidad alpha. El nombre de los índices, así como sus respectivas ecuaciones, pueden consultarse empleando la misma función, pero especificando el parámetro `help` como `TRUE` (`vegan::betadiver(help = TRUE)`).
...
beta <- lapply(
  ...
  function(idx) {
    ...
    beta_veg <- vegetacion %>%
      ...
      select(OBJECTID, valor, metodo, geom)
...
mapview(beta[["sor"]], zcol = "valor", layer.name = "Beta Sorensen") +
  mapview(beta[["j"]], zcol = "valor", layer.name = "Beta Whittaker")
...
El mapa de vegetación cedido por
