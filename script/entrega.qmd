...
names(mde) <- names(algoritmos_mde)
```
...
teselas_n <- lapply(
  names(algoritmos_mde),
  function(nombre) {
    opt_output_files(teselas) <- paste0(here::here(paste0("data/lidar/lidar2/norm/", nombre, "/", nombre, "_", "/")), "{ORIGINALFILENAME}") # Configurar salida del catálogo
    opt_laz_compression(teselas) # Configurar formato de salida

    # Cálculo del MDE
    teselas_n <- lidR::normalize_height(
      teselas,
      algoritmos_mde[[nombre]]
    )
...
mds <- lapply(
  names(algoritmos_mds),
  function(nombre) {
    # Cálculo del MDS
    mds <- lidR::rasterize_canopy(
      teselas_n,
      res = 1,
    ...
    terra::writeRaster(
      mds,
      here::here(paste0("data/mds/mds_", nombre, ".tif")),
      overwrite = TRUE
    )
    ...
    # Lista con los resultados
    return(list(full = mds, rec = mds_rec))
  }
)

# 2.4. Nombramos los elementos de la lista
names(mds) <- names(algoritmos_mds)
```
