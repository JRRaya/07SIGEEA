...
# 2.1. Lista de algoritmos a emplear
chm  <- terra::rast("data/mds/mds_tin_rec.tif")
treetop <- terra::rast("data/it/itd/itd_ws5.gpkg")
...
for(nombre in algoritmos_it) {
  ...
  sf::st_write(
    its,
    here::here(paste0("data/it/its/its_", nombre, ".gpkg")), 
    quiet = TRUE,
    delete_dsn = TRUE
  )
  ...
  sf::st_write(
    crown,
    here::here(paste0("data/it/crown/crown_", nombre, ".gpkg")), 
    quiet = TRUE,
    delete_dsn = TRUE
  )
...
```{r}
# 1. Cálculo de las "métricas del dosel"
# 1.1. Dispersión (Metrics characterizing the dispersion of point heights)
dispersion <- pixel_metrics(
  teselas_n,
  ~metrics_dispersion(z = Z, dz = 2, zmax = 56.5),
  res = 1
)

# 1.2. Rugosidad
rugosidad <- pixel_metrics(
  teselas_n,
  ~metrics_rumple(x = X, y = Y, z = Z, pixel_size = 1),
  res = 1
)

# 1.3. Textura (Generates a suite of GLCM (Grey-Level Co-Occurence Matrix) metrics of a canopy height model (CHM))
textura <- pixel_metrics(
  teselas_n,
  ~metrics_texture(x = X, y = Y, z = Z, pixel_size = 1),
  res = 1
)

# 2. Guardado de las capas
for(md in c("dispersion", "rugosidad", "textura")){
  writeRaster(
    get(md),
    here::here(paste0("data/md/", md, ".tif")),
    overwrite = TRUE
  )
}
```
